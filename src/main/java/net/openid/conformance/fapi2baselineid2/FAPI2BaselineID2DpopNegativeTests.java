...
public class FAPI2BaselineID2DpopNegativeTests extends AbstractFAPI2BaselineID2ServerTestModule {
	...
	class CallResourceEndpointSteps extends AbstractConditionSequence {
		...
		public CallResourceEndpointSteps(boolean expectSuccess, boolean shouldFail) {
		...
		public void evaluate() {
			...
			if (expectSuccess) {
				callAndContinueOnFailure(EnsureHttpStatusCodeIs200or201.class,result );
			} else {
				callAndContinueOnFailure(EnsureHttpStatusCodeIs401.class, result, "DPOP-7.1");
			}
	...
	protected void requestProtectedResource() {
		...
		call(new CallResourceEndpointSteps(false, true)
		...
		call(new CallResourceEndpointSteps(true, true)
			.insertAfter(SetDpopHtmHtuForResourceEndpoint.class,
				condition(AddQueryAndFragmentToDpopHtu.class)));

		eventLog.startBlock("Try DPoP proof where 'htu' is a different url");
		call(new CallResourceEndpointSteps(false, true)
			.insertAfter(SetDpopHtmHtuForResourceEndpoint.class,
				condition(SetDpopHtuToDifferentUrl.class)));
		...
		call(new CallResourceEndpointSteps(false, false)
			.insertAfter(CreateDpopClaims.class,
				condition(Fapi2DPoPNegativeConditions.FixedJtiClaim.class)));
		...
		eventLog.startBlock("Try DPoP proof with extra claims on header and footer");
		call(new CallResourceEndpointSteps(true, true)
			.insertAfter(SetDpopHtmHtuForResourceEndpoint.class,
		...
		eventLog.startBlock("Try DPoP proof expecting case independent comparison of htm claim");
		call(new CallResourceEndpointSteps(true, false)
			.replace(SetDpopHtmHtuForResourceEndpoint.class,
				condition(Fapi2DPoPNegativeConditions.DpopHtuUpperCase.class)));

		eventLog.startBlock("Try DPoP proof expecting Scheme based comparison of htm claim");
		call(new CallResourceEndpointSteps(true, false)
			.replace(SetDpopHtmHtuForResourceEndpoint.class,
